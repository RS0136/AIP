...
def run_sim_a4_local_temp(df: pd.DataFrame, out_dir: str, cores: int, seed: int = 0):
    ...
    # Pretty labels for A4 (must be added BEFORE saving)
    display_map = {
    ...
    # Now save table (CSV first, then optional TeX)
    try:
        df_tab.to_csv(base + ".csv", index=False)
    except Exception:
        pass
    ...
    # Pretty labels for A4 (used in figures and to align with paper text)
    display_map = {
        "beta_const_1.0": "const β=1.0",
        "beta_linear_1.0_to_1.5": "linear 1.0→1.5",
        "beta_linear_1.5_to_1.0": "linear 1.5→1.0",
    ...
    if "label" not in df_tab.columns:
        df_tab["label"] = [display_map.get(d, d) for d in df_tab["drift"].values]
...
def run_sim_model_recovery(df: pd.DataFrame, out_dir: str, cores: int, seed: int = 0):
    ...
    plt.xticks(np.arange(len(fit_cfgs)), [f for f, _ in fit_cfgs], rotation=30, ha="right")
    plt.yticks(np.arange(len(gen_cfgs)), [g for g, _ in gen_cfgs])
...
def run_sim_ladder_dll(df: pd.DataFrame, out_dir: str, cores: int, seed: int = 0):
    ...
    if not os.path.exists(ladder_csv):
        ...
        try:
            ...
            log(f"  Failed to run Sim-LADDER automatically: {e}")
    if not os.path.exists(ladder_csv):
    ...
    # Figure (no title; rely on axis labels/caption)
...
def run_sim_amb_entropy(df: pd.DataFrame, out_dir: str, cores: int, seed: int = 0):
    """
    Sim-AMB-ENTROPY: Ambiguity vs. Entropy check using out-of-fold listener posteriors.
    Outputs:
      - tables/table15_amb_entropy_detail.csv  (utterance-level: utterance, n_obs, p1_mean, entropy, ambiguity)
      - tables/table15_amb_entropy_summary.csv (global + per-context stats)
      - figures/fig15a_amb_entropy_scatter.(pdf|png)
      - figures/fig15b_amb_entropy_spearman_hist.(pdf|png)  (only if a context id is present)
    """
    ...
    # Save detail & summary tables
    tables_dir = os.path.join(out_dir, "tables"); ensure_dir(tables_dir)
    base_detail = os.path.join(tables_dir, "table15_amb_entropy_detail")
    try:
    ...
    base_summary = os.path.join(tables_dir, "table15_amb_entropy_summary")
    try:
